...
class TestHealth:
    ...
    async def test_postgres_down(self, client, monkeypatch):
        def bad_session():
            raise RuntimeError("PG down")
        monkeypatch.setattr("app.routes.monitoring.get_session", bad_session)
        r = await client.get("/health")
        assert r.status_code == 200
        assert r.json()["postgres"] == "disconnected"
        assert r.json()["status"]   == "degraded"

    async def test_both_down(self, client, orchestrator, monkeypatch):
