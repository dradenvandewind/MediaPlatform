...
class ManifestWriter:
    def __init__(self,
                 ready_video: set[int],
                 ready_audio: set[int],
                 ready_lock:  threading.Lock) -> None:
    ...
    def _get_windows(self) -> tuple[list[Path], list[Path]]:
        """
        Snapshot thread-safe : acquiert le lock pour copier les ready-sets,
        puis calcule les fenêtres en dehors du lock (I/O disque lente).
        """
        ...
        # Aligner sur le min pour éviter la dérive A/V dans les manifests
        n      = min(len(segs_v), len(segs_a))
    ...
    def _read_segment_duration_s(self, seg_path: Path) -> float:
        """Durée réelle via ffprobe, fallback SEG_DURATION_S."""
        try:
    ...
    def _write_mpd(self, segs_v: list[Path], segs_a: list[Path]) -> None:
        ...
        # Fallback PTO=0 si tfdt illisible (cmafmux peut omettre tfdt
        # ou le nester différemment). On logue une seule fois.
        ...
        # ← MODIFIÉ : PTO audio recalculé depuis la référence vidéo
        # pour garantir l'alignement A/V (évite le décalage tfdt inter-pistes)
        #pto_a_aligned = int(pto_s * timescale_a)
        pto_a_aligned = first_bmdt_v * timescale_a // timescale_v

        # ← MODIFIÉ : availabilityStartTime = wall-clock au moment du premier segment
        # On recule de pto_s pour que "maintenant" corresponde à la fin de la fenêtre
        #ast_epoch = datetime.now(timezone.utc).timestamp() - pto_s
        #ast_str   = datetime.fromtimestamp(ast_epoch, tz=timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        #     log.info("AST figé à %s (pto_s=%.1fs)", self._ast_locked, pto_s)
        
        if AST_FILE.exists():
            self._ast_locked = AST_FILE.read_text().strip()
        else:
            # PTO=0 → AST = maintenant − durée déjà écoulée depuis seg#1
            # first_seg_num = self._seg_number(segs_v[0])  
            # elapsed_s     = first_seg_num * SEG_DURATION_S  # approximation depuis le début
            # ast_epoch     = datetime.now(timezone.utc).timestamp() - elapsed_s
            # self._ast_locked = datetime.fromtimestamp(
            #     ast_epoch, tz=timezone.utc
            # ).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        # def make_seg_list(segs, init_file, timescale, duration_ts, pto):
        #     init_rel     = os.path.relpath(init_file, OUTPUT_DIR)
        #     start_number = self._seg_number(segs[0])
        #     seg_rel_dir  = os.path.relpath(segs[0].parent, OUTPUT_DIR)
            
        #     expected_bmdt = self._seg_number(segs_v[0]) * seg_duration_ts_v
        #     drift_frames  = first_bmdt_v - expected_bmdt
        #     log.debug(
        #         "PTO check: bmdt=%d  expected=%d  drift=%d  (%.3f s)",
        #         first_bmdt_v, expected_bmdt, drift_frames,
        #         drift_frames / timescale_v
        #     )
        ...
        mpd = (
            ...
            + make_seg_list(segs_v, INIT_FILE_VIDEO, timescale_v,
                            seg_duration_ts_v, first_bmdt_v) +
            '      </Representation>\n'
            '    </AdaptationSet>\n'
            ...
            + make_seg_list(segs_a, INIT_FILE_AUDIO, timescale_a,
                        seg_duration_ts_a, pto_a_aligned) +
            '      </Representation>\n'
            '    </AdaptationSet>\n'
            '  </Period>\n'
            '</MPD>\n'
...
class MultiFormatStreamer:
    ...
    def __init__(self):
        ...
        self._manifest_writer      = ManifestWriter(
            ...
            self._ready_audio,
            self._ready_lock,
            tream_start_epoch_ref=lambda: self._stream_start_epoch
        )
