...
def build_pipeline(on_segment_written) -> Gst.Pipeline:
    ...
    class SegmentFileWriter:
        ...
        def _handle(self, data: bytes):
            # Détecte les frontières ftyp/moov/moof
            pos = 0
            while pos + 8 <= len(data):
                size  = int.from_bytes(data[pos:pos+4], 'big')
                btype = data[pos+4:pos+8]
        ...
        def _flush(self):
            if not self._buf:
                return
            path = self._dir / f"seg{self._idx:05d}.m4s"
            path.write_bytes(bytes(self._buf))
            log.info("%s finalized → seg%05d.m4s (%d bytes)",
                    self._track, self._idx, len(self._buf))
            self._on_done(self._track, self._idx, path)
            self._buf.clear()
            self._idx += 1
...
class MultiFormatStreamer:
    ...
    def _on_segment_written(self, track: str, fragment_id: int, path: Path) -> None:
        """Appelé quand segment fragment_id est finalisé sur disque."""
        if track == "video":
            if not self._init_video_extracted and fragment_id == 0:
                self._extract_init(path, INIT_FILE_VIDEO)
                self._init_video_extracted = True
            self._ready_video.add(fragment_id)
            log.debug("Ready video: %d segments", len(self._ready_video))

        elif track == "audio":
            if not self._init_audio_extracted and fragment_id == 0:
                self._extract_init(path, INIT_FILE_AUDIO)
                self._init_audio_extracted = True
            self._ready_audio.add(fragment_id)
            log.debug("Ready audio: %d segments", len(self._ready_audio))
