...
class ManifestWriter:
    ...
    def _write_mpd(self, segs_v: list[Path], segs_a: list[Path]) -> None:
        ...
        mpd = (
            ...
            '     type="dynamic"\n'
            f'     availabilityStartTime="{self._availability_start_time}"\n'
            ...
            + make_seg_list(segs_a, INIT_FILE_AUDIO, timescale_a,
                            seg_duration_ts_a, first_bmdt_a) +
            '      </Representation>\n'
            '    </AdaptationSet>\n'
            '  </Period>\n'
            '</MPD>\n'
...
class MultiFormatStreamer:
    ...
    def _on_segment_written(self, track: str, fragment_id: int, path: Path) -> None:
        """
        Appelé par format-location-full quand splitmuxsink commence à écrire
        le segment fragment_id.  Le segment précédent (fragment_id-1) est
        alors totalement finalisé sur disque.

        Actions :
          1. Confirmer fragment_id-1 dans le ready-set (ManifestWriter pourra
             l'inclure dans les manifests).
          2. Extraire init.mp4 depuis seg00000.m4s lors du passage à
             fragment_id=1 (seg0 vient d'être finalisé).
        """
        if fragment_id == 0:
            # Premier segment en cours d'écriture — rien à confirmer encore.
            return

        prev_id = fragment_id - 1

        if track == "video":
            # Extraction de l'init à la fin du premier segment vidéo
            if prev_id == 0 and not self._init_video_extracted:
                seg0 = SEG_DIR_VIDEO / "seg00000.m4s"
                self._extract_init(seg0, INIT_FILE_VIDEO)
                self._init_video_extracted = True
            with self._ready_lock:
                self._ready_video.add(prev_id)
            log.debug("Ready video +%d  total=%d", prev_id, len(self._ready_video))

        elif track == "audio":
            if prev_id == 0 and not self._init_audio_extracted:
                seg0 = SEG_DIR_AUDIO / "seg00000.m4s"
                self._extract_init(seg0, INIT_FILE_AUDIO)
                self._init_audio_extracted = True
            with self._ready_lock:
                self._ready_audio.add(prev_id)
            log.debug("Ready audio +%d  total=%d", prev_id, len(self._ready_audio))
